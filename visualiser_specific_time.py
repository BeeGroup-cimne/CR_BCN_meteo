...
df = df.drop("weatherStation")
plot_air_temperature_raster(df, time_to_plot, max_area=0.000001, pdf_path=f"{plots_dir}/results_temperature_{time_to_plot}_new.pdf")

nextcloud_root_dir = os.path.expanduser('~/Nextcloud2/Beegroup/data/CR_BCN_meteo')
predictions_folder = f'{nextcloud_root_dir}/Historical_ERA5Land/Predictions_old'
y2 = int(str(ym)[0:4])
m2 = int(str(ym)[4:6])
if (m2-1) == 0:
    y2 = y2-1
...
df = df.drop("weatherStation")
plot_air_temperature_raster(df, time_to_plot, max_area=0.000001, pdf_path=f"{plots_dir}/results_temperature_{time_to_plot}_old.pdf")